                ...
                <div className="grid grid-cols-3 gap-2 mb-4">
                    {decisions.map((d) => (
                        <button
                            ...
                            className={`rounded-lg border-2 p-3 text-center transition-colors ${selected === d.value
                                    ? d.variant === "safe"
...
export default function CheckDetailPage() {
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                const data = await getCheck(tenantId, checkId);
    ...
    }, [tenantId, checkId, router]);

    if (!checkId) {
        return (
            <Card>
                <h1 className="text-lg font-semibold text-navy-900 mb-2">No check selected</h1>
                <p className="text-sm text-gray-600 mb-4">
                    Pick a check from the inbox to view its details.
                </p>
        ...
        <div className="max-w-3xl mx-auto space-y-6">
            ...
            <Card>
                <div className="flex items-center gap-4 mb-4">
                    <div
                        className={`h-12 w-12 rounded-full flex items-center justify-center text-lg font-bold ${check.verdict === "safe"
            ...
            {check.decision ? (
                ...
                <DecisionForm
                    checkId={check.id}
                    tenantId={tenantId!}
                    onDecided={(decision) => setCheck({ ...check, decision })}
                />
            )}
